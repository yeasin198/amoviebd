...
# --- [টেলিগ্রাম বট হ্যান্ডলার - আপনার অরিজিনাল লজিক ফিক্সড] ---
def register_handlers(bot_inst):
        ...
        if len(message.text.split()) > 1:
            ...
            if cmd_data.startswith('dl_'):
                ...
                try:
                    ...
                    bot_inst.send_message(message.chat.id, "❌ ফাইল পাওয়া যায়নি। স্টোরেজ চ্যানেল কনফিগারেশন চেক করুন।")
                return

        # স্বাগতম মেসেজ ও প্রোফাইল কার্ড
        welcome_text = (
    ...
    def cancel_process(message):
        ...
        if uid in admin_states:
            ...
            bot_inst.reply_to(message, "✅ বাতিল করা হয়েছে।")
    ...
    def stats(message):
        ...
        u_count, m_count = users_col.count_documents({}), movies_col.count_documents({})
    ...
    def post_search(message):
        ...
        bot_inst.send_message(message.chat.id, f"🔎 '{query}' এর জন্য লিঙ্ক জেনারেট করা হয়েছে।", reply_markup=markup)

    # --- সহায়ক চ্যাট হ্যান্ডলারসমূহ ---
...
def init_bot_service():
    ...
    if token and len(token) > 20:
        try:
            ...
            return bot
        except Exception as e: print(f"❌ Bot Initialization Failure: {e}")
    return None
...
def movie_details(tmdb_id):
    ...
    if 'files' in movie:
        for f in movie['files']:
            f['short_url'] = get_short_link(f"https://t.me/{bot_username}?start=dl_{f['file_id']}")
...
# ================== আপনার অরিজিনাল HTML টেমপ্লেটসমূহ ==================
...
<style>
    ...
    .btn-watch { background: var(--duple); color: #fff; padding: 10px 30px; border-radius: 30px; text-decoration: none; font-weight: 600; display: inline-block; margin-top: 15px; box-shadow: 0 4px 15px rgba(0, 210, 255, 0.4); }
    ...
    .cat-pill { padding: 6px 16px; border-radius: 20px; border: 1px solid var(--neon); color: var(--neon); text-decoration: none; margin: 4px; display: inline-block; transition: 0.3s; }
    .cat-pill.active { background: var(--neon); color: var(--dark); font-weight: bold; }
...
# ... [বাকি সব HTML টেমপ্লেট আপনার অরিজিনাল ফাইল থেকে হুবহু থাকবে] ...
ADMIN_SIDEBAR = """<div class="sidebar"><div class="sidebar-brand"><img src="{{config.SITE_LOGO}}" style="width:70px; border-radius:50%;"><h6>{{config.SITE_NAME}}</h6></div><a href="/admin?tab=dashboard">📊 Dashboard</a><a href="/admin?tab=add">➕ Add Content</a><a href="/admin?tab=movies">🎬 Movie List</a><a href="/admin?tab=settings">⚙️ Bot Settings</a><a href="/logout" style="color:#ff4d4d;">🚪 Logout</a></div>"""
ADMIN_DASHBOARD_HTML = f"<!DOCTYPE html><html><head><title>Admin</title><link rel='stylesheet' href='https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css'>{COMMON_STYLE}</head><body>" + ADMIN_SIDEBAR + """<div class="main-content"><h3>Welcome Admin</h3><div class="row"><div class="col-md-4"><div class="admin-card bg-primary text-white text-center"><h2>{{stats.users}}</h2><p>Users</p></div></div><div class="col-md-4"><div class="admin-card bg-success text-white text-center"><h2>{{stats.movies}}</h2><p>Movies</p></div></div></div></div></body></html>"""
ADMIN_ADD_HTML = f"<!DOCTYPE html><html><head><title>Add</title><link rel='stylesheet' href='https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css'><script src='https://code.jquery.com/jquery-3.6.0.min.js'></script>{COMMON_STYLE}</head><body>" + ADMIN_SIDEBAR + """<div class="main-content"><h3>Add New Content</h3><div class="admin-card" style="background:#1f2833; color:white;"><input id="url_in" class="form-control mb-3" placeholder="TMDb ID..."><button class="btn btn-info w-100 mb-4" onclick="fetchData()">Fetch Data</button><form action="/admin/manual_add" method="POST"><input id="f_title" name="title" class="form-control mb-2" placeholder="Title" required><input id="f_id" name="tmdb_id" class="form-control mb-2" placeholder="TMDB ID" required><select name="type" class="form-control mb-2"><option value="movie">Movie</option><option value="tv">TV</option></select><select name="category" class="form-control mb-2">{% for cat in categories %}<option value="{{cat}}">{{cat}}</option>{% endfor %}</select><input id="f_year" name="year" class="form-control mb-2"><input id="f_rating" name="rating" class="form-control mb-2"><input id="f_poster" name="poster" class="form-control mb-2"><textarea id="f_story" name="story" class="form-control mb-3" rows="3"></textarea><button class="btn btn-success w-100">Save</button></form></div></div><script>function fetchData(){ $.post('/admin/fetch_info', {url: $('#url_in').val()}, function(d){ $('#f_title').val(d.title); $('#f_id').val(d.tmdb_id); $('#f_year').val(d.year); $('#f_rating').val(d.rating); $('#f_poster').val(d.poster); $('#f_story').val(d.story); }); }</script></body></html>"""
ADMIN_MOVIES_HTML = f"<!DOCTYPE html><html><head><title>Movies</title><link rel='stylesheet' href='https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css'>{COMMON_STYLE}</head><body>" + ADMIN_SIDEBAR + """<div class="main-content"><h3>Movie Library</h3><div class="admin-card"><table class="table"><thead><tr><th>Poster</th><th>Title</th><th>Action</th></tr></thead><tbody>{% for m in movies %}<tr><td><img src="{{m.poster}}" width="40"></td><td>{{m.title}}</td><td><a href="/admin/edit/{{m.tmdb_id}}" class="btn btn-sm btn-warning">Edit</a> <a href="/delete/{{m.tmdb_id}}" class="btn btn-sm btn-danger">Del</a></td></tr>{% endfor %}</tbody></table></div></div></body></html>"""
ADMIN_SETTINGS_HTML = f"<!DOCTYPE html><html><head><title>Settings</title><link rel='stylesheet' href='https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css'>{COMMON_STYLE}</head><body>" + ADMIN_SIDEBAR + """<div class="main-content"><h3>Bot Settings</h3><div class="admin-card"><form action="/save_config" method="POST"><div class="row"><div class="col-md-6 mb-3"><label>Site Name</label><input name="site_name" class="form-control" value="{{config.SITE_NAME}}"></div><div class="col-md-6 mb-3"><label>Site URL</label><input name="site_url" class="form-control" value="{{config.SITE_URL}}"></div><div class="col-md-6 mb-3"><label>Bot Token</label><input name="token" class="form-control" value="{{config.BOT_TOKEN}}"></div><div class="col-md-6 mb-3"><label>TMDB Key</label><input name="tmdb" class="form-control" value="{{config.TMDB_API_KEY}}"></div><div class="col-md-6 mb-3"><label>Admin ID</label><input name="admin_id" class="form-control" value="{{config.ADMIN_ID}}"></div><div class="col-md-6 mb-3"><label>Channel ID</label><input name="channel_id" class="form-control" value="{{config.STORAGE_CHANNEL_ID}}"></div><div class="col-md-6 mb-3"><label>Shortener Link</label><input name="s_url" class="form-control" value="{{config.SHORTENER_URL}}"></div><div class="col-md-6 mb-3"><label>Shortener API</label><input name="s_api" class="form-control" value="{{config.SHORTENER_API}}"></div></div><button class="btn btn-primary w-100">Save</button></form></div></div></body></html>"""
EDIT_HTML = f"<!DOCTYPE html><html><head><title>Edit</title><link rel='stylesheet' href='https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css'>{COMMON_STYLE}</head><body>" + ADMIN_SIDEBAR + """<div class="main-content"><div class="row"><div class="col-md-6"><div class="admin-card"><h5>Edit Metadata</h5><form action="/admin/update" method="POST"><input type="hidden" name="tmdb_id" value="{{m.tmdb_id}}"><input name="title" class="form-control mb-2" value="{{m.title}}"><select name="category" class="form-control mb-2">{% for cat in categories %}<option value="{{cat}}" {% if m.category == cat %}selected{% endif %}>{{cat}}</option>{% endfor %}</select><input name="year" class="form-control mb-2" value="{{m.year}}"><textarea name="story" class="form-control mb-3">{{m.story}}</textarea><button class="btn btn-success w-100">Update</button></form></div></div><div class="col-md-6"><div class="admin-card"><h5>Add Link (Msg ID)</h5><form action="/admin/add_file" method="POST"><input type="hidden" name="tmdb_id" value="{{m.tmdb_id}}"><input name="quality" class="form-control mb-2" placeholder="720p Bangla"><input name="file_id" class="form-control mb-2" placeholder="Msg ID"><button class="btn btn-info w-100">Add</button></form><hr><h6>Current Links:</h6><ul>{% for f in m.files %}<li>{{f.quality}} (ID: {{f.file_id}}) <a href="/admin/delete_file/{{m.tmdb_id}}/{{f.file_id}}" class="text-danger">Del</a></li>{% endfor %}</ul></div></div></div></div></body></html>"""
...
BOT_SELECT_HTML = """<!DOCTYPE html><html><head><title>Select</title><link rel='stylesheet' href='https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css'></head><body class='bg-dark text-white p-4'><h4>Results for: {{query}}</h4>{% for i in results %}<div class='card bg-secondary mb-2 d-flex flex-row overflow-hidden'><img src='https://image.tmdb.org/t/p/w200{{i.poster_path}}' width='60'><div class='p-2'><b>{{i.title or i.name}}</b><br><a href='https://t.me/{{bot_username}}?start=sel_{{i.media_type}}_{{i.id}}' class='btn btn-sm btn-info'>Select</a></div></div>{% endfor %}</body></html>"""
